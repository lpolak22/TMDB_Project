...
export class FiltriranjeFilmovaComponent {
  ...
  async filtrirajFilmove() {
    ...
    try {
      let datumOdMillisekunde = this.datumOd ? new Date(this.datumOd).getTime().toString() : undefined;
      let datumDoMillisekunde = this.datumDo ? new Date(this.datumDo).getTime().toString() : undefined;
  
      const rezultat = await this.filtriranjeService.loadFilmovi(this.trenutnaStranica, datumOdMillisekunde, datumDoMillisekunde);
  
      this.filmovi = rezultat.filmovi || [];
      this.ukupanBrojFilmova = rezultat.ukupno || 0;
      this.ukupanBrojStranica = Math.ceil(this.ukupanBrojFilmova / this.brojFilmovaPoStranici);
