...
export class DodavanjeService {
  ...
  async pretraziOsobePoImenu(ime: string, stranica: number): Promise<any> {
    ...
    try {
      const response = await fetch(
        `${this.restServis}app/dodavanje?ime=${parametar}&stranica=${stranica}`,
        {
      ...
      console.error(error);
      throw new Error(
        "Žao nam je, došlo je do pogreške prilikom pretrage osoba."
      );
    }
  }
  ...
  async dohvatiFilmoveOsobe(id: number): Promise<any> {
    try {
      const response = await fetch(
        `${this.restServis}app/dodavanje/${id}/filmovi`,
        {
          headers: { "Content-type": "application/json" },
        }
      );
      ...
      console.error(error);
      throw new Error(
        "Žao nam je, došlo je do pogreške prilikom dohvata filmova osobe."
      );
    }
  }
  ...
  async dodajOsobuUBazu(podaciOsobe: any): Promise<void> {
    try {
      ...
      if (!Array.isArray(filmovi)) {
        throw new Error(
          "Rezultat funkcije dohvatiFilmoveOsobe nije niz filmova."
        );
      }
  ...
  async dodajFilmUBazu(podaciFilmova: any[]): Promise<void> {
    try {
      if (!Array.isArray(podaciFilmova)) {
        throw new Error(
          "Očekivan je niz podataka o filmovima, ali je dobiven drugačiji format."
        );
      }
      ...
      for (const film of podaciFilmova) {
        try {
          ...
          if (!response.ok) {
            const pogreskaInfo = await response.json();
            console.warn(
              `Film s ID-jem ${film.id} nije dodan: ${
                pogreskaInfo.greska || "Nepoznata greška"
              }`
            );
            continue;
          }
          ...
          console.warn(
            `Greška prilikom dodavanja filma s ID-jem ${film.id}:`,
            filmError
          );
          continue;
        }
  ...
  async poveziOsobuIFilm(osobaId: number, filmovi: any[]): Promise<void> {
    try {
      ...
      if (!response.ok) {
        const pogreskaInfo = await response.json();
        throw new Error(
          pogreskaInfo.greska || "Neuspješno povezivanje osobe s filmovima."
        );
      }
    } catch (error) {
      console.error(error);
  ...
  async obrisiOsobu(id: number): Promise<void> {
    try {
      const dohvaceniFilmovi = await fetch(
        `${this.restServis}osoba/${id}/film`,
        {
          method: "GET",
          headers: { "Content-Type": "application/json" },
        }
      ...
      if (!response.ok) {
        const pogreskaInfo = await response.json();
        throw new Error(
          pogreskaInfo.greska || "brisanje veze osobe na filmove nije uspjelo"
        );
      }
  ...
  async loadSlikeOsobe(id: number) {
    try {
      const response = await fetch(
        `${environment.restServis}app/detalji/${id}`,
        {
          headers: {
  ...
  private async posaljiSlikuUBazu(osobaId: number, slikaPutanja: string) {
    try {
      const response = await fetch(
        `${environment.restServis}app/detaljiSlike`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        console.error(
          "Greška pri dodavanju slike u bazu:",
          await response.text()
        );
      }
    } catch (error) {
