  ...
  async obrisiKorisnika(korime: string) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          pogreskaInfo.greska || "Brisanje korisnika nije uspjelo"
        );
      }
    } catch (error) {
  ...
  async dajPristup(korime: string) {
    try {
      const response = await fetch(
        `${this.restServis}app/korisniciPristup/${korime}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
      ...
      if (!response.ok) {
        const pogreskaInfo = await response.json();
        throw new Error(
          pogreskaInfo.greska || "Davanje pristupa nije uspjelo."
        );
      }
  ...
  async dodajKorisnika(korisnik: any) {
    try {
      ...
      if (!response.ok) {
        const pogreskaInfo = await response.json();
        throw new Error(
          pogreskaInfo.greska || "Dodavanje korisnika nije uspjelo"
        );
      }
    } catch (error) {
  ...
  async zabraniPristup(korime: string) {
    try {
      ...
      if (!response.ok) {
        const pogreskaInfo = await response.json();
        throw new Error(
          pogreskaInfo.greska || "Zabrana pristupa nije uspjela."
        );
      }
