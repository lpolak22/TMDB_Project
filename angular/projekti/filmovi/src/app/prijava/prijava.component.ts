...
export class PrijavaComponent {
  ...
  async prijaviSe(form: any): Promise<void> {
    ...
    try {
      ...
      const score = await this.prijavaService.provjeriRecaptcha(token);

      if (score < 0.7) {
        this.poruka = "Prijava blokirana zbog niskog reCAPTCHA rezultata.";
        return;
      }

      const korisnik = await this.prijavaService.prijaviKorisnika(
        korime,
        lozinka
      );
  ...
  async provjeriTOTP(form: any): Promise<void> {
    try {
      ...
      const rezultat = await this.prijavaService.provjeriTOTP(
        this.korime,
        this.totpKod
      );
      ...
      this.poruka =
        error instanceof Error
          ? `Greška: ${error.message}`
          : "Pogreška pri provjeri TOTP koda.";
    }
  }
}
