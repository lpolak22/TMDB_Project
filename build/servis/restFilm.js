...
export class RestFilm {
    ...
    async getFilmovi(zahtjev, odgovor) {
        ...
        try {
            ...
            let stranicaBroj = parseInt(stranica, 10) || 1;
            ...
            const sada = new Date();
            if (datumOd) {
                datumOdParsed = this.pretvoriUDate(datumOd);
            }
            if (datumDo) {
                datumDoParsed = this.pretvoriUDate(datumDo);
            }
            if (!datumOd && !datumDo) {
                odgovor.status(422).send({ greska: "neočekivani podaci" });
                return;
    ...
    async getSviFilmovi(zahtjev, odgovor) {
        ...
        try {
            ...
            const brojElemenata = 20;
            if (stranica) {
                stranicaBroj = parseInt(stranica, 10);
            ...
            let datumDoParsed = null;
            if (datumOd) {
                datumOdParsed = new Date(parseInt(datumOd, 10));
            ...
            if (datumDo) {
                datumDoParsed = new Date(parseInt(datumDo, 10));
